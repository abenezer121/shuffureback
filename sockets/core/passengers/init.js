...
module.exports = async (passengerInfo, passenger, socket) => {
  ...
  try {
    ...
    await updatePassenger(passenger._id, socket)({ socketId: socket.id, fcm: passengerInfo.fcm, position: passengerInfo.location })
    ...
    if (passenger && passenger.inActivePool && passenger.poolId) {
      ...
      const [rideRes, rentRes, requestRes, rentRequestResult] = await Promise.all([ride, rent, request, rentRequest])
      if (rideRes) {
