    ...
    if (data.route && data.pickUpAddress.name && data.dropOffAddress.name && data.pickUpAddress.coordinate && data.dropOffAddress.coordinate) {
      ...
      if (data.vehicle) {
        ...
        if (vhcl.online) {
          ...
          request.passenger = await getPassenger(request.passenger, "profileImage rating") // requester passenger's profile pic and rating are required
      ...
      Promise.all([pickup, dropOff]).then(value => {
        ...
        Axios.get(url).then(async (routeObject) => {
          ...
          //{ coordinates: routeObject.data.routes[0].geometry.coordinates, distance: routeObject.data.routes[0].distance, duration: routeObject.data.routes[0].duration }
