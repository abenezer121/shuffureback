    ...
    if (res) {
      if (res.status != TRIP_STATUS.COMPLETED) {
        ...
        if (res.passenger) {
          ...
          if (res.type !== TRIP_TYPES.ROAD_PICKUP && passengerTripCount === 1 && res.passenger.firstName != "_" && (tripDurationInMinutes >= 10 && tripDurationInMinutes < 180) && !res.dispatcher) {
            ...
            await session.withTransaction(async () => {
              await Incentive.create([{
                ...
                fare: res.fare,
                amount: 100,
                reason: `First trip miles (100 Birr)`
              }], { session: session })
