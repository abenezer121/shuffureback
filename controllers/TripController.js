...
const show = async (req, res) => {
  try {
    const trip = await Ride.findById(req.params.id).populate('driver').populate('vehicle').populate('vehicleType').populate('dispatcher').populate('passenger').populate('ticket').populate('dispatcherWhoEnded', 'firstName lastName')
    res.send(trip)
  } catch (error) {
    logger.error('Trip => ' + error.toString())
    res.status(500).send(error)
    ...
    Ride.findById(req.params.id, async (error, ride) => {
      ...
      if (ride) {
        if (ride.status != TRIP_STATUS.COMPLETED) {
          if (res.surge) {
            res.vehicleType.baseFare = res.vehicle.surgeBaseFare
            res.vehicleType.pricePerKM = res.vehicle.surgePricePerKM
            res.vehicleType.pricePerMin = res.vehicle.surgePricePerMin
          }
