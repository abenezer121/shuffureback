...
const income = (req, res) => {
  try {
    ...
    Promise.all([
      ...
      Ride.countDocuments({ driver: req.params.id, type: 'pool', endTimestamp: { $gte: today } }),
      Ride.countDocuments({ driver: req.params.id, type: 'bid', endTimestamp: { $gte: today } }),
      Ride.countDocuments({ driver: req.params.id, type: 'corporate', endTimestamp: { $gte: today } }),
      Ride.countDocuments({ driver: req.params.id, type: 'normal', endTimestamp: { $gte: today } }),
...
const store = async (req, res) => {
  try {
    ...
    if (req.body && req.body.firebaseToken && req.body.firstName && req.body.lastName && req.body.profileImage) {
      firebaseAuth()
...
module.exports = { index, auth, oldAuth, show, bookings, store, update, remove, rate, search, adminSearch, scheduledTrips, rents, topUp, walletHistory, income, updateWallet, walletTransfer, lend, buyAirTime, myVouchers, exportReport }
